...
> 旧 v5 = 第四版**，后续顺延。
> 磁盘目录与报告文件名保留旧编号（如 `risk_sft_v3`、`checkpoints\...-v5`），以本表映射为准。
> 模型重工件（checkpoint、merged 导出、GGUF）按约定清理；报告与 manifest 永久留痕。

## 主谱系

| 版次 | 旧编号 | 数据集 | 模型工件（可清理） | 报告文件（本目录） | 结果 |
| --- | --- | --- | --- | --- | --- |
| 第一版 | v1 | risk_sft_v1 / risk_sft_v2（720+120 配额制） | checkpoints\...-v1、exports\...-v1-merged（已清理） | `risk-qlora-eval-v1.json`（四轮重复评测已合并为一份，含验收块） | 验收失败：隐喻新增 +3 < 4；FPR 0 通过 |
| 第二版 | v3 | risk_sft_v3（840+140） | checkpoints\...-v3（已清理） | `risk-qlora-eval-v3.json`、`V3-TRAINING-EVAL-SUMMARY.md` | 8 门槛 7 过 1 败：FPR +4.84pp；隐喻新增 +8 |
| 第三版 | v4 | risk_sft_v4（1703+198+devtest 1414） | checkpoints\...-v4、exports\...-v4-merged | `risk-qlora-eval-v4.json`、`risk-qlora-devtest-v4.json`、`stress-eval-v4.log`、`devtest-eval-v4.log` | 8 门槛 7 过 1 败：FPR +3.23pp（差 1 条过线）；dev-test acc 0.910 |
| 第四版 | v5 | risk_sft_v5 | checkpoints\...-v5、exports\...-v5-merged | `risk-qlora-eval-v5.json`、`risk-qlora-devtest-v5.json`、`V5-TRAINING-EVAL-SUMMARY.md` | **八门槛全部通过（历史首次）**：FPR 0（零误升级）、隐喻 +8、P95 0.93s |
| 第五版 | v7 | risk_sft_v7 | checkpoints\...-v7、exports\...-v7-merged | `risk-qlora-eval-v7.json` | 7/8：第三人称 0.55→0.82、medium 召回 0.29→0.77；但 FPR 复发（corp-084/091 自我否定误升级） |
| 第六版 | v8 | risk_sft_v8 | checkpoints\...-v8、exports\...-v8-merged | `risk-qlora-eval-v8.json` | 7/8：acc 0.782、第三人称 0.91、medium 0.71；同样卡 corp-084/091 |
| **第七版** | **v9** | **risk_sft_v9（提示词 v2）** | checkpoints\...-v9、exports\...-v9-merged | `risk-qlora-eval-v9.json`、`risk-qlora-devtest-v9.json` | ✅ **八门槛全过 + 全指标最优**：FPR 0、corp-084/091 修复、medium 召回 0.88、acc 0.782、第三人称 0.82、隐喻 +6 |
...
> v2 编号作废：历史上不存在独立 v2 训练；原 `*-eval-v2.json` 为 v1 模型复评，
> 已并入 `risk-qlora-eval-v1.json` 后删除源文件。
...
| risk_sft_v3 | 第二版 | 840 train / 140 dev（280×3） | 七路：hongzhiq-suicide 380+57、socialcd 299+52、psysuicide 精选 71+11、synthetic 41+9、hard-negative 9+2、认知歪曲 10+3、project-base 30+6；隐喻 112+20 | 细节见 `data/risk_sft_v3/build_report.json` |
