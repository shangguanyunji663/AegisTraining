...
> 现状备注（2026-08-31）：当前代码中提示词的**独立物理副本为两处**——`data_contract.py`
> 是权威定义，数据构建（`prepare_risk_sft_v4.py`）、冻结评测与本地服务脚本均从此导入；
> 生产项目 `app/llm/client.py` 为另一份独立副本，须逐字同步。上文的"数据脚本"副本在
> 当时的脚本结构下存在，现已改为导入引用，自动保持一致。
